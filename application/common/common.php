 ...
 */
function openid2unionid($openid)
{
    if ($unionid = cache(CacheKey::OPENID_TO_UNIONID . $openid)) return $unionid;
    $api = getWechatApi();
    $user_wx = $api->getUserInfo($openid);
    $unionid = isset($user_wx['unionid']) ? $user_wx['unionid'] : '';
    if (!$unionid) return false;
    cache(CacheKey::OPENID_TO_UNIONID . $openid, $unionid);
    return $unionid;
}
