...
<div class="form-inline" align="center">
  ...
  <!--工具按钮结束-->
    <div class="form-group">
        <select name="admire_type" id="admire_type" class="form-control form-select" >
            <option value="">赞赏分类</option>
            <option value="1">Live直播赞赏</option>
            <option value="2">课程赞赏</option>
            <option value="3">观点赞赏</option>
        </select>
        <select name="open_status" id="open_status" class="form-control form-select" >
...
<script type="text/javascript">
    $table = $('#userListTable');
    var addUrl = './add';
    var g_delUrl = './del_array';
    var g_editUrl = './admire_edit';
    var url = './admire_list';
    ...
    //删除单条设置
    function admire_del(id){
        layer.confirm('确认要删除吗？',function(index){
            $.ajax({
                type: 'POST',
                url: '/AdmireSetup/admire_del?id='+id,
                dataType: 'json',
                success: function(data){
    ...
    //编辑设置
    function admire_edit(id) {
        layer_show('编辑赞赏', g_editUrl + '?id=' + id, '', g_windowHight);
    }
</script>