...
<div class="text-c" id="table-search">
    <form class="form form-horizontal" id="form-search" action="/CommentAudit/courseLeaveDetail" method="post">
        ...
        <h4 class="panel-title">
            <a data-toggle="collapse" data-parent="#accordion" href="#collapseOne" style="text-decoration: none;">
                <div>
                    ...
                    <a onclick="actionType(<?=$master['id']?>, 'changeLeaveStatus', <?=$master['state']?>, 2, 1)"><{if condition="$master.state == 1"}><span style="color: red">屏蔽</span><{else /}>显示<{/if}></a>
                    <a onclick="actionType(<?=$master['id']?>, 'replyLeave', <?=$master['groupid']?>, <?=$master['user_id']?>, 1)">回复</a>
                    <a onclick="actionType(<?=$master['id']?>, 'delLeave', 0, 1, 1)">删除</a>
                </div>
        ...
        <div class="panel-body">
            <a data-toggle="collapse" data-parent="#accordion" href="#collapseOne">
                <div>
                    ...
                    <a onclick="actionType(<?=$teacherId?>, 'leaveGossip', <?=$v['user_id']?>, 1, 1)">禁言</a>
                    <a onclick="actionType(<?=$v['id']?>, 'changeLeaveStatus', <?=$v['state']?>, 2, 2)"><{if condition="$v.state == 1"}><span style="color: red">屏蔽</span><{else /}>显示<{/if}></a>
                    <a onclick="layer.msg('前端暂不支持对回复再做回复')">回复</a>
                    <a onclick="actionType(<?=$v['id']?>, 'delLeave', 0, 1, 1)">删除</a>
                </div>
