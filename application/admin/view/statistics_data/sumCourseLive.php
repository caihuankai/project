...
<div class="form-inline" align="center">
    ...
    <div class="form-group">
        <input type="text" placeholder="课程名称" id="class_name" class="form-control input-text Wdate" name="create_time_min">
        <input type="text" placeholder="创建人" id="alias" class="form-control input-text Wdate" name="create_time_min">
        <select id='course_type' class="form-control form-select">
            <option value="">课程类型</option>
            <option value=1>单节课</option>
            <option value=2>系列课</option>
        </select>
        <input type="text" placeholder="开始时间(默认当天)" value="<?=date('Y-m-d',time())?>" onfocus="WdatePicker({ maxDate:'#F{$dp.$D(\'logmax\')||\'%y-%M-%d\'}' })" id="logmin" class="form-control input-text Wdate" name="create_time_min">
        -
        <input type="text" placeholder="结束时间" value="<?=date('Y-m-d',time())?>" onfocus="WdatePicker({ minDate:'#F{$dp.$D(\'logmin\')}',maxDate:'%y-%M-%d' })" id="logmax" class="form-control input-text Wdate" name="create_time_max">
...
<script type="text/javascript">
    ...
    function exportExcel(){
        ...
        var course_type = $.trim($('#course_type').val());
        var logmin = $.trim($('#logmin').val());
        var logmax = $.trim($('#logmax').val());
        var url = '/StatisticsData/sumCourseLiveExcel?logmin='+logmin+'&logmax='+logmax+'&class_name='+class_name+'&alias='+alias+'&course_type='+course_type;
