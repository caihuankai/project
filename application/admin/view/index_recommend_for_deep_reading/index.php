...
<div id="table-search-html">
    <input type="text" class="input-text" placeholder="观点标题" id="search-title" name="title"/>
    <{date_range title="添加"}>
...
<script>
    ...
    window['adminTableConfig'] = {
        onLoadSuccess: function (){
            ...
            //操作上的置顶/取消置顶按钮
            $('.open-top-status').click(function (){
            	var e = $(this)
                var id = e.data('id');
                var topStatus = e.data('top-status');
	            ...
	            var option = {
	                url: "<{:url('changeTopStatus')}>",
	                localSuccess: localSuccess
		        };
	            changeTopStatus(id, topStatus, option);
            });
    ...
    $(function () {
        ...
        $('#table-button-add').click(function (){
            ...
            layer_show('新增', '<{:url("add")}>', 1300, 600);
        });
    ...
    function changeTopStatus(ids, topStatus, option){
        requestAjax({
            ids: ids,
            topStatus: topStatus
        }, option);
