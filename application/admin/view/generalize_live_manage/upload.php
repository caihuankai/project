    ...
    <form class="form form-horizontal" id="form-admin-add">
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-3 col-sm-4">
                <select id="type" name="type">
                    <{volist name="$info['select']" id="vo"}>
                    <{if condition="$vo.text == $info['type']"}>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-4 col-sm-2">
                <img src="<{$info['src']}>" id="showimg"style="width: 60px;height: 60px;border: 0 solid #000">
            </div>
            <div class="formControls col-xs-4 col-sm-2">
                <a class="upload">上传
        ...
        <div class="row cl" id="sortbox" <{if condition="$info['type'] != 'right'"}> style="display: none"<{/if}> data-show"<{$info['type']}>">
            <label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>排序：</label>
            <div class="formControls col-xs-4 col-sm-6">
                <input type="number" class="input-text" placeholder="排序" id="sort" name="sort" <{if condition="($info['sort'] != null) AND ($info['type'] eq 'right')"}> value="<{$info['sort']}>"<{/if}> maxlength="30">
            </div>
        </div>
        <div class="row cl">
...
<script type="text/javascript">
    var postimg = null,Type=$("select#type").val();

    $("select#type").change(function (){
        Type=$("select#type").val();
        ElementIsShow($("#sortbox"),Type,'right');
    });
    ...
    $(function(){
        ...
        $("#form-admin-add").validate({
            ...
            submitHandler:function(form){
                ...
                postdata.push({name: "src", value:postimg});
                var url = "./upload";
                var jz;
                $.ajax({
                    type:"POST",
