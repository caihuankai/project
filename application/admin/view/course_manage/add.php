...
<article class="page-container">
    <form class="form form-horizontal" id="form-admin-add">
        ...
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>课程形式：</label>
            <div class="formControls col-xs-4 col-sm-4">
                <select id="form" name="form">
                	<{neq name="$seriesType" value="0"}>
                		<option value="2">视频直播</option>
                	<{else/}>
                    <option value="1">图文语音（普通）</option>
                    <option value="2">视频直播</option>
                    <option value="3">ppt直播</option>
                    <{/neq}>
                </select>
            </div>
        </div>
        <div class="row cl" id="isfree" <{eq name="$seriesType" value="0"}> style="display: none;" <{/eq}> >
        ...
        <div class="formControls col-xs-4 col-sm-4">
            <input type="password" class="input-text" value="" placeholder="请输入4位数英文和数字，部分大小写" id="password" name="password" maxlength="4">
        </div>
        （非必选，不填为空则不设密码）
        </div>
        <div class="row cl">
        ...
        <div class="row cl">
            <div class="col-xs-8 col-sm-3 col-xs-offset-3 col-sm-offset-3">
            	<input type="hidden" name="seriesType" value="<{$seriesType}>" >
...
<script type="text/javascript">
    var postimg = null,addUrl='<{:url("CourseManage/add")}>',
    ...
    $(function(){
        ...
        $("#form-admin-add").validate({
            ...
            submitHandler:function(form){
                var level =   $("#level option:selected").val(),origin_price= parseFloat($("#origin_price").val()),price= parseFloat($("#price").val());
                if (parseInt(level) != 0){
