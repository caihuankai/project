	...
	<div class="text-c">
	 <span class="select-box inline">
		...
		<!--
			...
			-->
			<option value="0" <{eq name="pdc_payment_state" value="0"}>selected<{/eq}>>待审批</option>
			<option value="1" <{eq name="pdc_payment_state" value="1"}>selected<{/eq}>>通过</option>
			<option value="2" <{eq name="pdc_payment_state" value="2"}>selected<{/eq}>>拒绝</option>
			<option value="3" <{eq name="pdc_payment_state" value="3"}>selected<{/eq}>>已汇款</option>
			<option value="4" <{eq name="pdc_payment_state" value="4"}>selected<{/eq}>>汇款失败</option>
		</select>
				...
				<tr class="text-c">
				  ...
				  <td class="td-status">
					 <{switch name="$vo.pdc_payment_state"}>
						...
						<{case value="4"}><span class="label label-success radius">汇款失败</span><{/case}>
					 <{/switch}>
				  </td>
				  <td><{$vo.remark}></td>
				  <td class="f-14 td-manage">
					  <{switch name="$vo.pdc_payment_state"}>
						...
						<{case value="2"}><a style="text-decoration:none;color: #999;" href="javascript:;" title="申请已拒绝">申请已拒绝</a><{/case}>
						<{case value="3"}><a style="text-decoration:none;color: #999;" href="javascript:;" title="提现完成">提现完成</a><{/case}>
						<{case value="4"}><a class="c-primary" onClick="article_start(this,<{$vo.pdc_id}>)" href="javascript:;" title="再次汇款">再次汇款</a><{/case}>
						<!--
						<{case value="4"}><a style="text-decoration:none;color: #999;"  href="javascript:;" title="汇款失败">汇款失败</a><{/case}>
						-->
					 <{/switch}>
...
function article_shenhe(obj,id){
	...
	function(){
		$.ajax({
			...
			success: function(data){
				if(data.status == 1){
					$(obj).parents("tr").find(".td-manage").prepend('<a class="c-primary" onClick="article_start(this,id)" href="javascript:;" title="汇款">汇款</a>');
	...
	function(){
		$.ajax({
			type: 'POST',
			url: '/PdCash/changePaymentState?paymentState=2&id='+id,
			dataType: 'json',
			success: function(data){
				if(data.status == 1){
					$(obj).parents("tr").find(".td-manage").prepend('<a style="text-decoration:none;color: #999;" href="javascript:;" title="申请已拒绝">申请已拒绝</a>');
...
function article_start(obj,id){
	layer.confirm('确认要汇款吗？',function(index){
		$.ajax({
			...
			success: function(data){
				if(data.status == 1){
					$(obj).parents("tr").find(".td-manage").prepend('<a style="text-decoration:none" onClick="article_stop(this,id)" href="javascript:;" title="确认已到账">确认已到账</a>');
