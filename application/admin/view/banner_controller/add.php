...
<article class="page-container">
    <form class="form form-horizontal" id="form-admin-add">
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>Banner名称:</label>
            <div class="formControls col-xs-4 col-sm-6">
                <input type="hidden" name="id" value="<{if condition="isset($id)"}> <{$id}> <{/if}>">
                <input type="text" class="input-text" value="<{if condition="isset($id)"}> <{$info['name']}> <{/if}>" placeholder="Banner名称" id="name" name="name" maxlength="30">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>上传图片：</label>
                <div class="formControls col-xs-4 col-sm-2">
                    <img src="<{if condition="isset($id)"}> <{$info['image']}> <{else /}> http://oqt46pvmm.bkt.clouddn.com/Ft56ta8EDC03VFYIHzFuSjcv5E5_<{/if}>"  id="showimg"style="width: 80px;height: 80px;border: 0 solid #000">
                </div>
            <div class="formControls col-xs-4 col-sm-2">
                <a class="upload">选择文件
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-4 col-sm-4">
                <input type="radio"  value="1" <{if condition="!isset($id) OR ($info['status'] eq 1)"}>checked="checked"<{/if}>  id="status" name="status">启用
            </div>
            <div class="formControls col-xs-4 col-sm-4">
                <input type="radio"  value="2" <{if condition="isset($id) AND ($info['status'] eq 2)"}>checked="checked"<{/if}> id="status" name="status">禁用
            </div>
        </div>
        <div class="row cl">
