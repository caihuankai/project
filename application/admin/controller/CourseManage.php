...
class CourseManage extends App
{
    ...
    public function index($type = 1, $action = 1)
    {
        ...
        $isChild = request()->param('pid',null);
        if ($type == 0) {
            $seriesBool = true;
        }
		//公众号H5用
        $seriesType = request()->param('seriesType/i', 0);
        ...
        if ($seriesBool) {//系列课列表的header
            ...
            if ($seriesType > 0) {//不显示 一级分类、二级分类、收费类型
            	unset($header[4]);
            	unset($header[5]);
            	unset($header['type']);
            }
            ...
            if (!empty($isChild)){
                array_pop($header);
                array_push($header,['field' => 'sort', 'title' => '排序',], ['field' => 'action', 'title' => '操作',]);
            }
        }
        if (!empty($userId)) {
        ...
        return $this->traitAdminTableList(function ()use($model, $action, $seriesSingleBool, $seriesBool, $seriesSingleParentOpenBool,$type,$isChild, $seriesType){

            if($type == 2){
                $field = 'c.id, c.uid, c.live_id, c.cate_id courseCate, c.class_name,c.price, c.level,
                        c.study_num, c.begin_time, c.create_time, c.open_status,c.sort, c.virtual_num, c.online_num, c.form, c.plan_num,
            ...
            if (!empty($isChild)){
                $order = 'c.open_status,c.sort asc,c.id desc';
            }else{
                $order = 'c.open_status,c.begin_time desc,c.id desc';
            }
            $data = $this->traitAdminTableQuery([], function ()use ($type, $model, $seriesBool, $seriesType){
                ...
                if ($type == 2){ // 用户购买的课程
                    ...
                    $model->where('c.seriesType', $seriesType);
                }else {
                    $model->whereSingleCourse();
                }
                
                //后台账号已绑定前端账号的情况下，仅显示自己的课程
                $adminBindUserId = $this->getAdminBindUserId();
                if ($adminBindUserId > 0) {
                	$model->where('c.uid', $adminBindUserId);
                }
                    ...
                    if ($action == 2){
                        ...
                        if (!$seriesBool) {//系列课不显示评论列表和推荐按钮
                            ...
                            if (!empty($isChild)){
                                $actionHtml['编辑排序'] = [
                                    'class'    => "dataSort",
                                    'src' =>  "javascript:_setSort({$datum['id']},{$datum['sort']});",
                                ];
                            }
                        }elseif ($seriesType == 0) {
                            $actionHtml['推荐'] = [
        ...
        }, function () use ($model, $userId, $type, $seriesType){
            $str = '';
            switch ($type){
                case 1:
                    $str = '直播课程数据';
            ...
            $this->assign('seriesType', $seriesType);
        });
    }
    ...
    {
        $id = trim(input('dataCopy'));
        $model = new \app\admin\model\Course();
        $datum=$model->where('c.id',$id)->alias('c')
            ->join('user u', 'u.user_id = c.uid', 'left')
            ->field('c.id,c.goal,c.src_img,u.alias,c.class_name')
            ->find();
        ...
        $copyData = [
            'id'=>$datum['id'],
            'type'=>$copyType,//1课程，2观点
            'class_name'=>$datum['class_name'],
            'abstract'=>$datum['goal'],
            'alias'=>$datum['alias'],
            'src_img'=>$datum['src_img'],
        ];
    ...
    public function add()
    {
        ...
        if (request()->isPost()){
            ...
            $coursePid = trim(request()->param('coursePid',0));
            //获取直播间ID
            ...
            $saveData = [
                ...
                'type'=>$courseType,
                'form'=>trim(request()->param('form/i',1)),
            ...
            if (intval($courseType) == 1) {//系列课子课的处理
            	if ($saveData['pid'] > 0) {
            		$pidData = $model->getCourseInfo($saveData['pid'], 'price,level,cate_id,class_name,seriesType');
            		// 继承系列课
            		$saveData['price'] = $pidData['price'];
            		$saveData['level'] = $pidData['level'];
            		$saveData['cate_id'] = $pidData['cate_id']; // 系列课的单次课程继承系列课
            	}
            }
            $model->db()->startTrans();
            try{
                ...
                $model->db()->commit();
                if ($result){
                	if ($coursePid > 0 && $pidData['seriesType'] > 0) {
                		//已购买系列课更新子课程
                		$replaceArray = [
                		...
                		$userIdList = (new \app\wechat\model\PayOrder())->getUserIdsByCourse($coursePid);
                		\MessageCenter::instance()->createMessageRecords('COURSE_SERIES_H5_CREATE_CHILD_TO_BUY_USER', ['courseId'=>$coursePid,'level'=>$saveData['level'],'form'=>$saveData['form']], $replaceArray, $userIdList);
                	}
    ...
    public function details($id)
    {
        ...
        if ($isPost){ // 保存
            ...
            $field = 'id, uid, live_id, cate_id, class_name, teacher_name, lecturers, src_img, price, info_video_qg_id,introduction_img_id, introduction_code_id,
            begin_time, create_time, level, password, brief, study_num,goal,virtual_study_num,virtual_num, pid, type, plan_num, form, origin_price';
        }
...
//            }
            ...
            $save = [
                ...
                'introduction_img_id' => $request->post('introductionImgId/i', 0),
            ];
    ...
    public function setChildCourseSort($id,$sort)
    {
        if (empty($id))return $this->errSysJson('ID不能为空');
        $courseModel = new \app\admin\model\Course();
        $courseModel->db()->startTrans();
        try{
            $result = $courseModel->save(['sort'=>$sort],['id'=>$id]);
            $courseModel->db()->commit();
            if ($result){
