...
class InstitutesRecommend extends App{
	/**
	...
	public function seriesColumn($positionType = 19){
        ...
        if(request()->isPost()){
            ...
            $name = $param['name'];
            $adStatus = $param['adStatus'];
            $list = $adsModel->dataList($page,$size,$name,$adStatus,$param['logmin'],$param['logmax'], $positionType,$param['relevanceType'],$param['remark']);
            if(!empty($list)){
                foreach ($list as $k => $v) {
                	switch ($v['relevanceType']) {
						case 7:
    ...
    public function addRecomment($positionType){
        if(request()->isPost()){
            ...
            if($positionType == 13){
                $data['remark'] = $param['baseFile'];
            }
            if(empty($param['baseFile']) && !in_array($positionType,[21,18,16,15,12,10])){
            ...
            elseif(empty($param['adStatus'])){
                ...
                if(in_array($positionType,[21,18,16,15,12,10])){
                    $courseM = new Course();
                    $courseInfo = $courseM->where('id',$param['adURL'])->find();
                    if(empty($courseInfo)){
    ...
    public function editRecomment($positionType=0){
        ...
        if(request()->isPost()){
            ...
            //banner类型需要上传APP图片
            if(in_array($param['positionType'],[1,9,14,20,17])){
                if(empty($param['appBaseFile'])){
                    return $this->error('请选择APP图片');
                }
                $updateData['remark'] = $param['appBaseFile'];
            }
            $getInsertInfo = $this->handleUrl($param['relevanceType'],$param['adURL']);
    ...
    public function addSeriesColumn(){
        ...
        if(request()->isPost()){
            ...
            $data = $adsModel->getWOAAddSeriseColumnList($page,$size,$param['course_id'],$param['teacher_name'],$param['course_name'],$param['remark'],$param['course_type']);
    ...
    public function addPositionType11($positionType=11){
        if(request()->isPost()){
            ...
            elseif(empty($param['adURL']) && $param['relevanceType'] != 13){
                return $this->error('请填写链接');
            }
            elseif(empty($param['adSort'])){
                return $this->error('请填写排序');
            ...
            elseif(empty($param['adStatus'])){
                return $this->error('请选择状态');
            }elseif (empty($param['videoUrl']) && $param['relevanceType'] == 13) {
                return $this->error('请上传视频');
            }else{
                $data['type'] = 1;
                if($param['relevanceType'] == 13){
                    $data['adURL'] = $param['videoUrl'];
                    $data['relevanceId'] = $param['videoUrl'];
                }else{
                    $getInsertInfo = $this->handleUrl($param['relevanceType'],$param['adURL']);
                    $data['adURL'] = $getInsertInfo['url'];
                    $data['relevanceId'] = $getInsertInfo['id'];
        ...
        if(request()->isPost()){
            ...
            elseif(empty($param['adStatus'])){
                ...
                if($param['relevanceType'] == 13){
                    ...
                    $updateData['relevanceId'] = $param['videoUrl'];
                }else{
                    $getInsertInfo = $this->handleUrl($param['relevanceType'],$param['adURL']);
                    $updateData['adURL'] = $getInsertInfo['url'];
                    $updateData['relevanceId'] = $getInsertInfo['id'];
                }
    ...
    public function handleUrl($data_type,$type_id){
		switch ($data_type) {
			...
			case 4:
				$data['id'] = $type_id;
				$data['url'] = config('WX_DOMAIN').'/#/personalCenter/course/'.$type_id;
				break;
			case 9:
				$data['id'] = $type_id;
				$data['url'] = config('WX_DOMAIN').'/#/personalCenter/course/'.$type_id;
			...
			case 10:
				$data['id'] = $type_id;
				$data['url'] = config('WX_DOMAIN').'/#/specialTrainAdvance/'.$type_id;
				break;
			case 11:
				$data['id'] = $type_id;
				$data['url'] = config('WX_DOMAIN').'/#/course/specialOnlive/'.$type_id;
				break;
			case 12:
				$data['id'] = $type_id;
				$data['url'] = config('WX_DOMAIN').'/#/review/'.$type_id;
				break;
            case 6:
                $data['id'] = $type_id;
                $data['url'] = config('WX_DOMAIN').'/#/live/'.$type_id.'&0&4';
                break;
			default:
				$data['id'] = '';
				$data['url'] = '';
