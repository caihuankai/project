...
class JiahuaExpression extends App
{
    ...
    public function index()
    {
        ...
        $search = [
            ...
            [['options' => ['data' => $model->getDataArr('jtype')], 'name' => 'table-search-type'], 'select', ],
        ...
        return $this->traitAdminTableList(function () {
            ...
            $data = $this->traitAdminTableQuery([], function () use ($model) {
                $model->where(['e.status'=>['in', [1,2]],'type'=>2])
                    ->alias('e')
            ...
            foreach ($data['rows'] as $k => $datum) {
                ...
                $result[] = [
                    ...
                    'type' =>$model->getDataStatus('jtype',$datum['type']),
                    'sort' => $datum['sort'],
    ...
    public function add()
    {
        if (request()->isPost())
        {
            ...
            $checkname = $model->where(['name'=>$name,'type'=>2,'status'=>['in',[1,2]]])->find();
    ...
    public function edit()
    {
        ...
        if (request()->isGet())
        {
            $id = $this->trimdata('id');
            $field = 'e.id,e.expression_number,e.name,e.type,e.sort,e.status,e.create_time,e.action_name,e.pic_id,qi.qiniuImg';
            $info = $model->getExpressionData($field,3,$id,2);
        ...
        if (request()->isPost())
        {
            ...
            $checkname = $model->where(['name'=>$name,'type'=>2,'status'=>['in',[1,2]]])->where('id','<>',$uid)->find();
            if (count($checkname)>0) return $this->errSysJson(['code'=>400,'msg'=>"该表情名已存在！"]);
