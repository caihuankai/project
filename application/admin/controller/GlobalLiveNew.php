 ...
 * Fashion:
 */
class GlobalLiveNew extends App
{
    ...
    public function detailsadd()
    {
        ...
        if (request()->isGet()){
            ...
            //验证时间可否预定
            $sql = "SELECT id,count(1) as num FROM `talk_global_live` WHERE open_status = 1 AND ( ( `set_start_time` <= '$startTime' AND `set_end_time` > '$startTime' ) OR ( `set_start_time` < '$endTime' AND `set_end_time` >= '$endTime' ) OR ( `set_start_time` >= '$startTime' AND `set_end_time` <= '$endTime' ) )";
