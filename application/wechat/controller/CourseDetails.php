...
use app\wechat\model\CoursePptImg;
use Qiniu\Auth;
...
class CourseDetails extends App{
	...
	public function details($user_id,$id){
		...
		if($courseInfo['type'] == 2){//系列课
			...
			$class_type = 2;
			if ($courseInfo['seriesType'] > 0 ) {
				$class_type += $courseInfo['seriesType']/10;
			}
		...
		if($courseInfo['level'] == 2){
			...
			if($courseInfo['pid'] != 0){
				...
				$parentSeriesType = $CourseModel->where('id',$courseInfo['pid'])->value('seriesType');
				if ($parentSeriesType == 1) {
					$type = 10;
				}elseif ($parentSeriesType == 2) {
					$type = 11;
				}
			}else{
			...
			$isBuyResult = $PayOrderModel->isBuy($user_id, $type, $class_id);
			if (is_array($isBuyResult)) {
				$userBuyStatus = $isBuyResult[0];
				$overdueTime = $isBuyResult[1];
			}else {
				$userBuyStatus = $isBuyResult;
			}
		}
		...
		$data = array(
            ...
            "app_id" => $id,
			"overdueTime"=>$overdueTime//有效期，月度课/季度课时用
		);
	 ...
	 * 编辑课程
	 * @param  integer $id           课程id
	 * @param  string  $src_img      课程封面图
	 * @param  string  $class_name   课程名称
	 * @param  string  $password     直播间密码
	 * @param  string  $teacher_name 主讲人
	 * @param  string  $lecturers    主讲人介绍
	 * @param  string  $brief        课程简介
	 * @param  array   $img_brief    课程图文介绍
	 * @param  string  $player_img   播放器默认图片
	 * @param  integer $plan_num	 计划课程数
	 * @param  decimal $price        课程价格（目前仅系列课可修改）
	 * @param  array   $class_sort   课程排序
	 * @return [type]                [description]
	 */
	public function editCourse($id = 2,$src_img = '',$class_name = '',$password = '',$teacher_name = '',$lecturers = '',$brief = '',$img_brief = array(),$player_img = '',$plan_num = 0,$price = 1,$class_sort = array()){
		$CourseModel = new Course();
		$LiveImgModel = new LiveImg();
		$user_id = $this->getUserId();
		// $user_id= 1706743;
		$id = (int)$id;
		$class_type = $CourseModel->where('id',$id)->value('type');
		if($class_type == 2){
			$data['price'] = $price;
		}
		...
		// $img_brief = array(array("src"=>"111","explain"=>"111"),array("src"=>"222","explain"=>"222"));
		//$class_sort = array(array("id"=>3,"sort"=>11),array("id"=>29,"sort"=>22));
		$LiveImgStatus = $LiveImgModel->saveCourseImg($id,$img_brief,2);
		$status = $CourseModel->edit($id,$data,$class_sort,$player_img,$user_id);
		if($status['code'] == 0){
			return $this->successJson($status['msg']);
		}else{
			return $this->errorJson(JsonResult::ERR_CLASS_EDIT);
		}
	}
	//删除课程 接受参数课程id $id
	public function  deleteCourse($id = 3){
		$id = (int)$id;
		$CourseModel =  new Course();
		$status = $CourseModel->deleteCourse($id);
		if($status['code'] == 0){
			return $this->successJson($status['msg']);
		}else{
			return $this->errorJson(JsonResult::ERR_CLASS_DELETE);
		}
	}
	 ...
	 * ppt课程操作
	 * @param  [type] $id      课程id
	 * @param  [type] $type    操作类型 1：新增 2：删除 3：编辑排序
	 * @param  [type] $content 操作内容	对应type 1:新增内容 2：图片id 3：编辑内容
	 * @param  [type] $client  1：安卓 2：ios
	 * example 
	 * 1:array($medie_id,$medie_id,$medie_id,$medie_id);
	 * 2:图片id
	 * 3:array(array("id"=>图片id,"sort"=>序号),array("id"=>图片id,"sort"=>图片序号));
	 * @return [type]          [description]
	 */
	public function pptCourseOperation($id=2,$type=1,$content=82,$client=0){
		//$id = 2;
		//$type = 1;
		// $content = array('O9-pLhsDb8ASM8pFAsZbw980Vie6y3sAohEeT3Zq-6jG6eJO8JY9xCGjf7kKzr1W15G','fJ9oGNu84bW8aGRn_JlwDPCaqiWE5SQPRvkUiBMhsVTr6HLnxa9pinH3iRG2G5yu');
		$user_id = $this->getUserId();
		// $user_id = 1706743;
		// $content = 82;
		$CoursePptImgModel = new CoursePptImg();
		//批量新增图片
		$qiniuKeyArray = array();
		...
		if($type == 1){
			for ($i=0; $i < count($content); $i++) { 
				if($client != 0){
					$qiniuKey = $content[$i];
					$imgUrl = 'http://os700oap7.bkt.clouddn.com/'.$qiniuKey;
				}else{
					$qiniuKey = $this->uploadImg($content[$i]);
					$imgUrl = 'http://os700oap7.bkt.clouddn.com/'.$qiniuKey;
				}
				if(!empty($qiniuKey)){
					$qiniuKeyArray[] = $qiniuKey;
					$data = $CoursePptImgModel->where('qiniuKey','in',$qiniuKeyArray)->where('course_id',$id)->select();
					if(empty($data)){
						$saveDate[] = ['course_id' => $id,'user_id' => $user_id,'qiniuKey' => $qiniuKey,'imgUrl' => $imgUrl,'type' => 1];
					}
				}
			}
			if(!empty($saveDate)){
				$state = $CoursePptImgModel->saveAll($saveDate);
			}else{
				return $this->errorJson(JsonResult::ERR_IMG_UPLOAD_REPEAT);
			}
			//上传成功通知c++
			if($state){
				$AdmireRpcC->sendPPTware($id,1,$qiniuKeyArray);
				return $this->sucSysJson('新增成功');
			}
			return $this->errorJson(JsonResult::ERR_IMG_UPLOAD);
		}
		if($type == 2){
			$where['course_id'] = $id;
			$where['id'] = $content;
			// $where['status'] = 1;
			$qiniuKey = $CoursePptImgModel->where($where)->value('qiniuKey');
			//图片是否已删除
			if(empty($qiniuKey)){
				return $this->errorJson(JsonResult::ERR_IMG_NOT_EXISTS);
			}
			$data = $CoursePptImgModel->where($where)->select();
			//删除对应图片
			$state = $CoursePptImgModel->where($where)->delete();
			$AdmireRpcC->sendPPTware($id,2,$data);
			//删除成功通知C++和前端
			if($state){
				return $this->sucSysJson('删除成功');
			}else{
				return $this->errorJson(JsonResult::ERR_IMG_DELECT);
			}
		}
		if($type == 3){
			if(!empty($content)){
				foreach ($content as $k => $v) {
					if(!empty($v)){
						$save[] = ['id' => $v['id'],'sort' => $v['sort'],'update_time' => date('Y-m-d H:i:s',time())];
					}
				}
			}
			$state = $CoursePptImgModel->saveAll($save);
			//修改序号成功通知c++和前端
			if($state){
				$AdmireRpcC->sendPPTware($id,3,0);
				return $this->sucSysJson('编辑成功');
			}else{
				return $this->errorJson(JsonResult::ERR_IMG_UPDATE_SORT);
			}
		}
	}
	/**
	 ...
	 * @return [type]     [description]
	 */
	public function courseware($id){
		$CoursePptImgModel = new CoursePptImg();
		$data = $CoursePptImgModel->getCourseware($id,1);
	...
	public function uploadImg($media_id){
		// 构建鉴权对象
        $auth = \Qiniu::instance('-A_E2mEZ14NOTYAq9dSR-Uhg4tAtT-HVJWX1m4m2','AgNMfRQT12HrKoSOKKwsYe8N49TBlvaTNe9Pjtjh','qiaoliaohd')->getAuth();
     	// 要上传的空间 0531
		$bucket = \Qiniu::instance('-A_E2mEZ14NOTYAq9dSR-Uhg4tAtT-HVJWX1m4m2','AgNMfRQT12HrKoSOKKwsYe8N49TBlvaTNe9Pjtjh','qiaoliaohd')->getBucket();
        // 生成上传 Token
        $upToken = $auth->uploadToken($bucket);
        //获取access_token
        $accessTokenModel = new \app\wechat\model\AccessToken();
        $access_token = $accessTokenModel->limit(1)->value('access_token');
        //微信服务器上图片url
        $wx_img = "http://file.api.weixin.qq.com/cgi-bin/media/get?access_token=".$access_token."&media_id=".$media_id;
        //获取图片
        $wx_img_content = file_get_contents($wx_img);
        $fetch = base64_encode($wx_img_content);
        $imggg = (new \app\wechat\controller\QiNiu())->request_by_curl(\Qiniu::instance('-A_E2mEZ14NOTYAq9dSR-Uhg4tAtT-HVJWX1m4m2','AgNMfRQT12HrKoSOKKwsYe8N49TBlvaTNe9Pjtjh','qiaoliaohd')->getPutB64Url(), $fetch, $upToken);
