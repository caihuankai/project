...
use app\admin\model\GlobalLive;

class CourseCorrelation extends App
{
    protected $noLoginAction = [
        'getLanclass','getGlobalLiveToday','getCurrentTeachInfo',
    ];
     ...
     */
    public function getLanclass()
    {
        $model = new GlobalLive();
...
//        $dataList = $backgroundImg;
                ...
                if ((strtotime($currenttime)>= strtotime($NightStartTime) && strtotime($currenttime)<= strtotime($NightEndTime))&& $checkCourse == true){
                    ...
                    if ($LiveDate['video_pic_id'] != 0 && !empty($LiveDate['video_id'])){
                        ...
                        $dataList = [
                            ...
                            'picUrl' =>$qiNiuModel->getQiNiuUrl($LiveDate['video_pic_id'],true),
                            'videoUrl'=>$qiNiuModel->getQiNiuUrl($LiveDate['video_id'],true),
                        ];
                    }else{
                        //课程表
                        $dataList = [
    ...
    public function getGlobalLiveToday()
    {
        ...
        foreach ($dataList as $data) {
            ...
            if (strtotime($currenTime)>= strtotime($data['set_start_time']) && strtotime($currenTime) <= strtotime($data['set_end_time']))
            {
                $data['playstatus']=2;
            }elseif(strtotime($currenTime)>= strtotime($data['set_start_time'])&& strtotime($currenTime) >= strtotime($data['set_end_time'])){
                $data['playstatus']=1;
            }elseif ((strtotime($currenTime)<= strtotime($data['set_start_time']))&& strtotime($currenTime) <= strtotime($data['set_end_time'])){
                $data['playstatus']=3;
            }
            ...
            $data['startTime']=date('H:i',strtotime($data['set_start_time']));
            $data['endTime']=date('H:i',strtotime($data['set_end_time']));
            $result[] = $data;
        }
