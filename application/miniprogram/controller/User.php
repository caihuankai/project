...
//        dump($this->request->get());
        ...
        try{
        	...
        	if (!empty($redirectUrl)) {
        		if ($this->ifWeChatBrowser()) {//微信浏览器访问，直接跳转
    ...
    protected function registerUserMiniprogramLoginSuccess($token, $userId, $oAuthData = null)
    {
    	// 更新token
    	if (!empty($token)) {
    		$this->initMiniprogramSessionId($token);
    	}
    	
    	$this->registerUserSuccessFunc($userId, $oAuthData);
    	
    	\think\Log::record('小程序登录成功，token值：' . $token, 'debug');
