...
abstract class Connection
{
    ...
    protected $config = [
        ...
        // 是否开启慢查询记录
        'slow_log'       => false,
        // 慢查询记录基准值(SQL执行超过该值会独立记录到sql_slow.log日志里)
        'slow_time'      => 1
    ];
    ...
    protected function trigger($sql, $runtime, $explain = [])
    {
        if (!empty(self::$event)) {
            ...
            // 未注册监听则记录到日志中
            // 超过自定义的慢查询时间,单独记录到slow_sql.log
            if ($this->config['slow_log'] && ($runtime >= $this->config['slow_time'])) {
                $requestPath = \think\Request::instance()->url();
                $row = '[ '. $requestPath. ' ] ' . "\n";
                $row .= '[ '. date('Y-m-d H:i:s',time()) . ' SLOW-SQL ] ' . $sql . ' [ RunTime:' . $runtime . 's ]' . "\n\n";
                $runtimePath = RUNTIME_PATH .'log/' . date('Ym',time()) . '/slow_sql.log';
                error_log($row,3, $runtimePath);
            }else{
