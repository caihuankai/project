...
class WxPayApi
{
    ...
    public static function unifiedOrder($inputObj, $timeOut = 6)
    {
        ...
        $inputObj->SetSpbill_create_ip(self::get_real_ip()); // 终端ip
     ...
     */
    public static function unifiedOrder_mini($inputObj, $timeOut = 6)
    {
        $url = "https://api.mch.weixin.qq.com/pay/unifiedorder";
        // 检测必填参数
        if (!$inputObj->IsOut_trade_noSet()) {
        ...
        $inputObj->SetAppid(config('wechat.mini_program_app_id')); // 公众账号ID
        $inputObj->SetMch_id(config('wechat.MCHID')); // 商户号
        $inputObj->SetSpbill_create_ip(self::get_real_ip()); // 终端ip
        // $inputObj->SetSpbill_create_ip("1.1.1.1");
        $inputObj->SetNonce_str(self::getNonceStr()); // 随机字符串
    ...
    public static function unifiedMobileOrder($inputObj, $timeOut = 6)
    {
        $url = "https://api.mch.weixin.qq.com/pay/unifiedorder";
        // 检测必填参数
        if (!$inputObj->IsOut_trade_noSet()) {
        ...
        $inputObj->SetSpbill_create_ip(self::get_real_ip());
        $inputObj->SetNonce_str(self::getNonceStr()); // 随机字符串
     ...
     * @date 2017/05/25
     */
    public static function cashoutOrder($inputObj,$timeOut = 15){
        $url = "https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers";
        // $inputObj['nonce_str'] = self::getNonceStr();
        // 检测必填参数
        if (empty($inputObj['partner_trade_no'])) {
            throw new WxPayException ("缺少统一支付接口必填参数partner_trade_no！");
        } else if (empty($inputObj['check_name'])) {
            throw new WxPayException ("缺少统一支付接口必填参数check_name！");
        } else if (empty($inputObj['amount'])) {
            throw new WxPayException ("缺少统一支付接口必填参数amount！");
        } else if (empty($inputObj['desc'])) {
            throw new WxPayException ("缺少统一支付接口必填参数desc！");
        }
        $inputObj['nonce_str'] = self::getNonceStr();//获取随机字符串
        $inputObj['spbill_create_ip'] = self::get_real_ip(); // 终端ip
        // 签名
        ksort($inputObj);
        $str='';
        ...
        $inputObj['sign'] = md5($str);

        // return $str;die;
        $xml = self::arraytoxml($inputObj);
        // return $xml;die;
        $response = self::postXmlCurl($xml, $url, true, $timeOut);
        $result = WxPayResults::InitCashout($response);//将xml转为array
