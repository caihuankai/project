...
class app extends \EasyWeChat\Foundation\Application
{
    ...
    public function __construct($extendConfig = 1)
    {
        ...
        if ($extendConfig === 3){ // app
        	...
        	$config = array_merge($config, $config['open-web']);
        }
    ...
    {
        return self::getSysConfig('open-web')['app_id'];
    }
    ...
    public function getOauthData($bool = false)
    {
        ...
        if (empty($userData) || $bool) { // 未登录
            ...
            if ($redirectUrl) {
                ...
                //指定登陆状态同步页面，该链接由运维配置到特定控制器
                $synchroPage = $request->param('synchroPage', '');
                if (!empty($synchroPage)) {
                	$urlJoin .= '&synchroPage=' . urlencode($synchroPage);
                }
            } else if (!self::$ajaxBool) { // ajax不获取HTTP_REFERER
                $redirectUrl = $request->server('HTTP_REFERER');
            }
            $registerUserUrl = url('User/registerUser', '', true, \helper\UrlSys::getWxHost()); // php的注册url
//             $registerUserUrl = url('User/registerUser', '', true, request()->domain()); // php的注册url
