 ...
 */
    ...
    (function () {
        ...
        div.innerHTML = "  <link/><table></table><a href='http://www.99soft.com/a'>a</a><input type='checkbox'/>";
...
(function ($) {
    var WriteLimit = function (element, options) {
        ...
        if (elem.data('max')) {
            settings.max = elem.data('max');
        }
        if (elem.data('unit')) {
            settings.unit = elem.data('unit');
        }
        if (elem.data('direction')) {
            settings.direction = elem.data('direction');
        }
        if (elem.data('counter')) {
            settings.counter = $(elem.data('counter'));
        }
...
(function (e, t, i) {
    ...
    var w, g, v, y, x, b, T, C, k, E, H, W, F, L, S, M, R, I, K, P, B, O, _, D, A, N, z, j, U, $, q, G, Q, J, V, X = {
            ...
            onCleanup: !1,
            onClosed: !1
        }, Y = "colorbox", Z = "cbox", et = Z + "Element", tt = Z + "_open", it = Z + "_load", ot = Z + "_complete",
        nt = Z + "_cleanup", rt = Z + "_closed", ht = Z + "_purge", lt = e("<a/>"), st = "div", at = 0, dt = {},
        ct = function () {
            function e() {
                clearTimeout(h)
            }
    ...
    e.colorbox || (e(f), Q = e.fn[Y] = e[Y] = function (t, i) {
        ...
        if (U) {
            ...
            H.empty().remove(), H = o(st, "LoadedContent").append(i), H.hide().appendTo(W.show()).css({
                ...
                }, L.html(B.title).add(H).show(), a > 1 ? ("string" == typeof B.current && S.html(B.current.replace("{current}", z + 1).replace("{total}", a)).show(), R[B.loop || a - 1 > z ? "show" : "hide"]().html(B.next), I[B.loop || z ? "show" : "hide"
                    ]().html(B.previous), ct(), B.preloading && e.each([r(-1), r(1)], function () {
    ...
    $.fn[pluginName] = function (options) {
        return this.each(function () {
            ...
            if (!(obj = $.data(this, pluginName))) {
                var $this = $(this), data = $this.data(), opts = $.extend({}, options, data);
...
;(function ($, undefined) {
    ...
    function Panes($wrapper, options) {
        ...
        this.$pane_wrapper.on('select.' + pluginName, function (e, options, index) {
            ...
            self.$panes.eq(index).addClass(type)[0].offsetWidth;
    ...
    $.extend(Panes.prototype, {
        select: function (index) {
            ...
            var tab_count = this.$panes.length, next;
    ...
    $[pluginName].defaults = {active: 0, easing: ''}
})(jQuery);
;(function ($, undefined) {
    "use strict";
    ...
    function Tab($header_wrapper, options) {
        ...
        var self = this, auto_click = false;
        ...
        this.$header_wrapper.on('click.' + pluginName, 'a', function (e) {
            var $this = $(this), $my_li = $this.closest('li'), my_index = $my_li.index();
    ...
    $.fn[pluginName].defaults = {active: 0, mode: 'next', easing: ''};
    $(function () {
...
(function ($) {
    $(document).ready(function () {
        $("#mobile > li").mouseenter(function () {
            $("#mobile .libox").removeClass('active');
            $(this).find('.libox').addClass('active');
            $("#mobileimg > img").css({'display': 'none'});
            $("#mobileimg > img").eq($(this).index()).stop().fadeIn(800);
        });
        $('#slide-b2c').slidesjs({width: 1200, height: 420, navigation: false, pagination: false});
        $('#slide-b2b2c, #slide-b2b, #slide-o2o').slidesjs({
            width: 1200,
            height: 660,
            navigation: false,
            pagination: false
        ...
        $('.title-list li').mouseenter(function () {
            ...
            $('.information-con-list .tab-pane').removeClass('active')
            $('.information-con-list .tab-pane').eq($(this).index()).addClass('active')
        });
        $('.title-list li:first').trigger('mouseenter');
        $('.top-tabs a').click(function (e) {
            e.preventDefault();
            $(this).tab('show')
        });
        $('.top-tabs a:first').trigger('click');
        ...
        var Text = $('.tel-kf-share-top').find('.text'), Icon = $('.tel-kf-share-top').find('.icon'),
            shItem = $('.tel-kf-share-top').find('.item-wrap'), shLi = $('.tel-kf-share-top').find('.item'),
            up = $('.tel-kf-share-top').find('.top');
        shItem.hover(function () {
            var Parent = $(this).parent();
            Parent.addClass('current');
            var thisText = $(this).find('.text');
            var TextW = thisText.innerWidth();
            console.log('TextW',TextW);
            thisText.stop().animate({'left': -TextW}, 300, function () {
            });
        }, function () {
            var Parent = $(this).parent();
            var thisText = $(this).find('.text');
            thisText.stop().animate({'left': '50px'}, 300, function () {
        ...
        $("#navid .drop").hover(function () {
            var _thisindex = $(this).index()
            $(".header-sec-menu > div").stop().fadeOut(400);
            $(".header-sec-menu").find("#dropdown" + _thisindex).fadeIn(400);
            $(".header-sec-menu").stop().animate({"bottom": -87}, 800);
        }, function () {
            $(".header-sec-menu").stop().animate({"bottom": 330}, 800);
        })
        $(".header-sec-menu").hover(function () {
            $(".header-sec-menu").stop().animate({"bottom": -87}, 800);
        }, function () {
            $(".header-sec-menu").stop().animate({"bottom": 330}, 800);
        })
        ...
        $("#slides").slidesjs({
            ...
            height: 700,
            pagination: false,
            play: {active: false, auto: true, interval: 5000, swap: false, pauseOnHover: true, restartDelay: 2500}
        });
        ...
        $('.process-choose-link').hover(function () {
            var Parent = $(this).parent(), Sec = $('.sec-item');
            Parent.find(Sec).stop().fadeIn(800);
        }, function () {
            $('.sec-item').stop().fadeOut(800);
        });
        var slideFn = function (node) {
            var caseSlide = $('.appcase-con'), caselist = caseSlide.find('.appcase-con-list'),
                caseitems = caselist.find('.appcase-con-item'), step = 4;
            left = 0, length = caseitems.length, isAnimate = true, index = 0, Next = $('.next'), Prev = $('.prev');
            var itemWidth = caseitems.outerWidth(true);
            var TitleItem = $('.appcase-title-item');
            caselist.width(length * itemWidth);
            TitleItem.eq(0).addClass('current');
            var currentIndex = 0;

            function nextFn(index) {
                TitleItem.removeClass('current').eq(index).addClass('current');
                caselist.stop().animate({left: -step * itemWidth * index}, 1000, function () {
                    currentIndex = index;
                    isAnimate = true;
                });
            }

            function prevFn(index) {
                TitleItem.removeClass('current').eq(index).addClass('current');
                caselist.stop().animate({left: -step * itemWidth * index}, 1000, function () {
                    currentIndex = index;
                    isAnimate = true;
        ...
        slideFn('#appcase-con');
        var Cover = $(".solutions-item-wrap").find("span");
        Cover.mouseover(function () {
            $(this).stop().fadeOut(400);
        });
        $(".solutions-item-wrap").mouseleave(function () {
        ...
        $('#header-div1').find('.header-div2').hover(function () {
            $(this).siblings('.header-div2').removeClass('active');
            $('#header-div2').find('.header-div4').css('display', 'none');
            $(this).addClass('active');
            $('#header-div2').find('.header-div4').eq($(this).index()).css('display', 'block');
        })
        $('#advertising .close').click(function () {
            $('#advertising').hide();
        });
        $('.tel-kf-share-top ul li .item-wrap .consult').mouseout(function () {
            $(this).find('span').css({'background-color': '#848484', 'opacity': '0.5'});
        });
        $('.tel-kf-share-top ul li .item-wrap .consult').mouseover(function () {
            $(this).find('span').css({'background-color': '#ff7852', 'opacity': '1'});
        });
        $('.pf_serviceList').hover(function () {
            $(this).addClass('mouseOn')
        }, function () {
            $(this).removeClass('mouseOn')
        });
        $('.caseBox').hover(function () {
            $(this).find('.eject').fadeIn(400)
        }, function () {
            $(this).find('.eject').fadeOut(400)
        });
    });
})(jQuery)
+ function ($) {
    ...
    Tab.prototype.show = function () {
        ...
        var hideEvent = $.Event('http://www.99soft.com/combined/min/hide.bs.tab', {relatedTarget: $this[0]})
        var showEvent = $.Event('http://www.99soft.com/combined/min/show.bs.tab', {relatedTarget: $previous[0]})
        ...
        this.activate($target, $target.parent(), function () {
            $previous.trigger({type: 'http://www.99soft.com/combined/min/hidden.bs.tab', relatedTarget: $this[0]})
            $this.trigger({type: 'http://www.99soft.com/combined/min/shown.bs.tab', relatedTarget: $previous[0]})
        })
    }
    ...
    function Plugin(option) {
        return this.each(function () {
            var $this = $(this)
            var data = $this.data('http://www.99soft.com/combined/min/bs.tab')
            if (!data) $this.data('http://www.99soft.com/combined/min/bs.tab', (data = new Tab(this)))
